...
with tf.variable_scope('conv1') as scope:
    ...
    # or you can use a simple use of this
...
with tf.variable_scope('conv2') as scope:
    ...
    # or you can use a simple use of this
    # conv2 = tf.contrib.layers.conv2d(pool1, 64, 5, 1, activation_fn=tf.nn.relu, padding='SAME')
...
with tf.variable_scope('fc') as scope:
    # use weight of dimension 7 * 7 * 64 * 1024
    input_features = 7 * 7 * 64
    w = tf.get_variable('weights', [input_features, 1024], initializer=tf.truncated_normal_initializer())
    b = tf.get_variable('biases', [1024], initializer=tf.random_normal_initializer(0.0))

    # reshape pool2 to 2 dimensions
    pool2 = tf.reshape(pool2, [-1, input_features])
    fc = tf.nn.relu(tf.matmul(pool2, w) + b, name='relu')

    # or you can use this
    # pool2 = tf.contrib.layers.flatten(pool2)
    # fc = tf.contrib.layers.fully_connected(pool2, 1024, tf.nn.relu)
...
utils.make_dir('/home/hiter/Git/checkpoints')
utils.make_dir('/home/hiter/Git/checkpoints/convnet_mnist')
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('/home/hiter/Git/my_graph/mnist', sess.graph)
    ...
    if isTrain:
        ...
        for index in range(initial_step, n_batches * N_EPOCHS): # train the model n_epochs times
            ...
            if (index + 1) % SKIP_STEP == 0:
                ...
                total_loss = 0.0
                saver.save(sess, '/home/hiter/Git/checkpoints/convnet_mnist/mnist-convnet', index)
        ...
        ckpt = tf.train.get_checkpoint_state(os.path.dirname('/home/hiter/Git/checkpoints/convnet_mnist/checkpoint'))
